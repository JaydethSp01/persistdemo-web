...
export function CrudTable({
  ...
  const KEY = "sd_crud_" + (storageKey || title).toLowerCase().replace(/\s+/g, "_");
  ...
  // cargar de localStorage al montar (persistencia real entre recargas)
  useEffect(() => {
    try {
  ...
  // guardar en cada cambio (solo tras la carga inicial, para no pisar lo guardado)
  useEffect(() => {
